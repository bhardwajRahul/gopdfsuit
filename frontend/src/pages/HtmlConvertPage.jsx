 ...
 * (identical input tabs and preview toggle; only the config panel and
 * result kind differed) behind {mode}. Both tabs render via WASM.
 */
const HtmlConvertPage = ({ mode = 'pdf' }) => {
  ...
  const convertViaServerConsent = async () => {
    ...
    const pageUrl = fallbackOffer.url
    setFallbackOffer(null)
    await run({
      endpoint,
      body: JSON.stringify({ ...config, url: pageUrl }),
  ...
  return (
    ...
    >
      <ConsentBanner offer={fallbackOffer} onConsent={convertViaServerConsent} onDismiss={() => setFallbackOffer(null)} isLoading={isLoading} actionLabel={isPdf ? 'Upload URL to server and convert to PDF' : 'Upload URL to server and convert to image'} />
        ...
        <div className="glass-card" style={{ padding: '2rem' }}>
          ...
          {showPreview && inputType === 'html' && htmlContent && (
            <div style={{ marginTop: '1.5rem' }}>
              ...
              <iframe srcDoc={htmlContent} style={{ width: '100%', height: '320px', border: '1px solid rgba(255,255,255,0.15)', borderRadius: '8px', background: 'white' }} title="Preview" />
            </div>
          )}
        </div>
      </div>
      {(resultUrl || isLoading) && (
        <div style={{ marginTop: '1.5rem' }}>
          <OperationShell
            ...
            emptyTitle={isPdf ? 'PDF preview will appear here' : 'Image preview will appear here'}
            emptySubtitle="Enter HTML or a URL, then run the conversion."
            onDownload={() => download(filename)}
            downloadLabel={isPdf ? 'Download PDF' : 'Download Image'}
            kind={isPdf ? 'pdf' : 'image'}
            imageAlt="Generated"
            isLoading={isLoading}
            loadingLabel={isPdf ? 'Converting to PDF…' : 'Converting to Image…'}
          />
        </div>
      )}
